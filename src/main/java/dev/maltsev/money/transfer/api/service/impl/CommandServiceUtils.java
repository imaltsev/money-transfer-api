...
import dev.maltsev.money.transfer.api.service.exception.InvalidTransferRequestException;
...
public class CommandServiceUtils {
    protected static void validateTransaction(Transaction transaction, Connection connection) throws InvalidTransferRequestException {
        ...
        if (payerAccount.balance().compareTo(transaction.amount()) < 0) {
            throw new InvalidTransferRequestException(String.format("Insufficient funds in account '%s'", transaction.payerAccountNumber()));
        }
    }

    static void validateTransferTransaction(Transaction transaction, Connection connection) throws InvalidTransferRequestException {
